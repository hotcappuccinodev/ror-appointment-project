  ...
  context 'Create Reservation ' do
    before(:each) do
     ...
     it 'Create reservation as authorized user' do
      post api_v1_vehicle_reservations_path(Vehicle.first.id),params:{authentication_token: User.first.authentication_token, date: '24-09-2022', city: 'bravos'}
      data = JSON.parse(response.body)
      expect(data['status']).to eq('Success')
    end

     it 'Create reservation as invalid atrributs' do
      post api_v1_vehicle_reservations_path(Vehicle.first.id),params:{authentication_token: User.first.authentication_token, date: '24-09-2022'}
      data = JSON.parse(response.body)
